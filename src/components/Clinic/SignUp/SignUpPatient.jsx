// src/components/Clinic/SignUp/SignUpPatient.jsx
          ...
          return (
            <div key={s.key} className="flex-1 flex items-center">
              <button
                type="button"
                onClick={() => setStep(i)}
                className={`h-8 min-w-8 px-2 rounded-full text-xs font-medium inline-flex items-center gap-2 pr-4
...
export default function SignUpPatient({ onSubmit }) {
  ...
  const logo = clinic?.logo_url ? `${API_URL}/${clinic.logo_url}` : null;
  ...
  // Removed strict password length check as requested
  const canNextFromStep0 = fullName.trim() && dateNaissance;
  const canSubmit = email && password && confirm && password === confirm && terms;
            ...
            <div className="flex items-center gap-3">
              <div className="h-9 w-9 rounded-xl grid place-items-center bg-white border border-slate-200">
                {logo ? (
                  ...
                  <img src={logo} className="h-9 w-9 rounded-xl object-cover" />
                ) : (
              ...
              <div className="leading-tight">
                ...
                <h1 className="text-base font-semibold text-slate-900">{clinic.name}</h1>
              </div>
            </div>
            ...
            {step === 1 && (
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div className="grid gap-4 md:grid-cols-2 md:col-span-2">
                  ...
                  {/* Password strength removed as requested */}
